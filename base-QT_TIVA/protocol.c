...
int32_t frame_stuffing(uint8_t *origen, uint8_t *destino,int32_t longitud, int32_t longitud_maxima)
{
	int32_t i,j;
	uint8_t tmp;
...
int add_checksum_and_stuff(uint8_t *frame,int32_t size, int32_t max_size)
{
	uint16_t checksum;
	uint8_t *ptrtochar;
	...
	frame[size+1]=(uint8_t)(checksum>>8);	//Añade el checksum
	size+=CHECKSUM_SIZE;
