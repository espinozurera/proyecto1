		    	...
		    	case COMANDO_TEMP:
		    			{
		    			    ...
		    			    if (check_command_param_size(i16Numdatos,sizeof(parametro)))
		    			    {
		    			    	extract_packet_command_param(pui8Frame,sizeof(parametro),&parametro);
		    			    		...
		    			    		HWREG(ADC0_BASE + ADC_O_PC) = (ADC_PC_SR_125K);// Tasa de muestreo (max) Tambien se puede hacer
		    			    		...
		    			    		ADCIntClear(ADC0_BASE, 1); // Limpia el flag de interrupción del ADC
		    			    		...
		    			    		parametro.temp=ui32TempValueC;
		    			    		uint8_t pui8Frame[MAX_FRAME_SIZE];
		    			    		i16Numdatos=create_frame(pui8Frame,COMANDO_TEMP,&parametro,sizeof(parametro),MAX_FRAME_SIZE);
		    			    		if (i16Numdatos>=0)
		    			    					{
		    			    						xSemaphoreTake(mutexUSB,portMAX_DELAY);
		    			    						send_frame(pui8Frame,i16Numdatos);
